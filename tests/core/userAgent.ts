...
import userAgent = require('nova/core/userAgent');
import has = require('nova/core/has');

registerSuite(function () {
	var suite = {
		name: 'nova/core/userAgent',

		beforeEach: function () {
		},
		'test useragent': function() {
			var list: {[index: string]: any} = {
			...
			for (let ua in list) {
				let expected = list[ua];
				//remove all entries to prevent test colision
				for (let name in expected) {
					has.add(name, undefined, true);
				}

				userAgent.process(ua);

				for (let name in expected) {
					let value = expected[name];
					assert.equal(userAgent(name), value, 'tested property: ' + name + ' -- tested useragent: ' + ua);

					has.add(name, undefined, true); //remove all entries to prevent test colision
				}
			}
